...
pub async fn build_with_overrides(
    ...
    let account_purge_repo: Arc<dyn ind_application::repos::account_purge::AccountPurgeRepository> =
        Arc::new(ind_persistence::repos::PgAccountPurgeRepository::new(
            pool.clone(),
        ));
    ...
    let mut allowed_redirect_uris = config.extension.redirect_uris.clone();
    allowed_redirect_uris.push("com.useindelible.app:/oauth/callback".to_string());
