...
impl ServerConfig {
    ...
    ///   EXTENSION_REDIRECT_URIS (comma-separated),
    ...
    fn load_from_env(env: &impl EnvSource) -> anyhow::Result<Self> {
        ...
        let builder = Config::builder()
            ...
            .set_default("auth.allow_signups", false)?
            ...
            // Passthrough streams assets from the API origin, so a self-hoster needs
            // no publicly reachable object store. Presigned makes the asset proxy
            // redirect to URLs signed against S3_ENDPOINT verbatim; only enable it
            // when that endpoint is reachable by browsers (e.g. real AWS S3).
            ...
            .set_default("extension.redirect_uris", default_extension_redirect_uris())?;
        ...
        let mut cfg: Self = builder
            ...
            .set_override_option(
                "extension.redirect_uris",
                parse_csv(env, "EXTENSION_REDIRECT_URIS"),
            )?
        ...
        cfg.extension.redirect_uris =
            validate_extension_redirect_uris(&cfg.extension.redirect_uris)?;
        cfg.validate(is_dev)?;
        Ok(cfg)
    }
...
fn validate_extension_redirect_uris(values: &[String]) -> anyhow::Result<Vec<String>> {
    if values.is_empty() {
        anyhow::bail!("EXTENSION_REDIRECT_URIS must contain at least one callback URL");
    }
    ...
    for value in values {
        if value.contains('*') {
            anyhow::bail!("EXTENSION_REDIRECT_URIS must not contain wildcards: {value}");
        }

        let url = Url::parse(value).map_err(|error| {
            anyhow::anyhow!("invalid EXTENSION_REDIRECT_URIS value {value}: {error}")
        })?;
        if url.scheme() != "https" {
            anyhow::bail!("EXTENSION_REDIRECT_URIS must use https: {value}");
        }
        if url.host_str().is_none() {
            anyhow::bail!("EXTENSION_REDIRECT_URIS must include a host: {value}");
        }
        if !url.username().is_empty() || url.password().is_some() {
            anyhow::bail!("EXTENSION_REDIRECT_URIS must not include credentials: {value}");
        }
        if url.query().is_some() || url.fragment().is_some() {
            anyhow::bail!("EXTENSION_REDIRECT_URIS must not include a query or fragment: {value}");
        }

        let canonical = url.to_string();
        if !seen.insert(canonical.clone()) {
            anyhow::bail!("EXTENSION_REDIRECT_URIS contains a duplicate callback: {canonical}");
        }
