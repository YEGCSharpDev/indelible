...
impl ObsidianPreviewRenderer {
    ...
    pub fn validate_settings(settings: &ObsidianExportSettings) -> Result<(), AppError> {
        let mut settings = settings.clone();
        settings.group_files_in_category_folders = true;
        ...
        for item_type in [
            ...
            document.item_type = item_type;
            crate::obsidian::render_document(
                &settings,
                &document,
        ...
        crate::obsidian::render_sync_notification(&settings, 1, now)
            .map_err(template_validation_error)?;
        Ok(())
    }
...
fn template_validation_error(error: crate::obsidian::ObsidianRenderError) -> AppError {
    AppError::Domain(DomainError::Validation {
        field: "template".into(),
...
#[cfg(test)]
mod tests {
    use super::ObsidianPreviewRenderer;
    use ind_application::AppError;
    use ind_domain::{DomainError, ObsidianExportSettings};

    fn assert_invalid_template(
        mut configure: impl FnMut(&mut ObsidianExportSettings),
        expected_renderer: &str,
    ) {
        let mut settings = ObsidianExportSettings {
            group_files_in_category_folders: false,
            sync_notifications: false,
            ..Default::default()
        };
        configure(&mut settings);

        let error = ObsidianPreviewRenderer::validate_settings(&settings).unwrap_err();
        match error {
            AppError::Domain(DomainError::Validation { field, message }) => {
                assert_eq!(field, "template");
                assert!(
                    message.contains(&format!("template `{expected_renderer}`")),
    ...
    #[test]
    fn settings_validation_renders_every_stored_template_even_when_inactive() {
        assert_invalid_template(
            |settings| settings.file_name_template = Some("{{ title".into()),
            "file_name",
        );
        assert_invalid_template(
            |settings| settings.properties_template = Some("{{ title".into()),
            "properties",
        );
        assert_invalid_template(
            |settings| settings.page_title_template = "{{ title".into(),
            "page_title",
        );
        assert_invalid_template(
            |settings| settings.metadata_template = "{{ title".into(),
            "metadata",
        );
        assert_invalid_template(
            |settings| settings.highlight_header_template = "{{ title".into(),
            "highlight_header",
        );
        assert_invalid_template(
            |settings| settings.highlight_template = "{{ highlight_text".into(),
            "highlight",
        );
        assert_invalid_template(
            |settings| settings.sync_notification_template = "{{ document_count".into(),
            "sync_notification",
        );
    ...
    #[test]
    fn settings_validation_accepts_valid_custom_templates() {
        let mut settings = ObsidianExportSettings {
            group_files_in_category_folders: true,
            sync_notifications: true,
            properties_template: Some("title: {{title}}".into()),
            page_title_template: "# {{title}}".into(),
            metadata_template: "Category: {{category}}".into(),
            highlight_header_template: "## Highlights on {{date}}".into(),
            highlight_template: "- {{highlight_text}} ({{color}})".into(),
            file_name_template: Some("{{author}} - {{title}}".into()),
        ...
        ObsidianPreviewRenderer::validate_settings(&settings).unwrap();
    }
}
