...
pub const TEST_ASSET_COOKIE_SECRET_HEX: &str =
...
fn test_config(base_url: &str, options: &TestAppOptions) -> ind_api::config::ServerConfig {
    ...
    let mut config = serde_json::json!({
        ...
        "cors": {"origins": [frontend_url], "frontend_url": frontend_url},
    ...
    if let Some((feed_domain, library_domain)) = options.email_ingest_domains.as_ref() {
        config["email_ingest"] = serde_json::json!({
            "feed_domain": feed_domain,
            "library_domain": library_domain
        });
    }
    if !options.credential_key_configured
...
#[derive(Debug, Clone)]
pub struct TestPersonalAccessToken {
    token: String,
}

impl TestPersonalAccessToken {
...
pub trait TestApiCredential {
    fn bearer_token(&self) -> &str;
}

impl TestApiCredential for TestAuthSession {
    fn bearer_token(&self) -> &str {
        &self.token
    }
}

impl TestApiCredential for TestPersonalAccessToken {
    fn bearer_token(&self) -> &str {
        &self.token
    }
}
...
impl TestApp {
    ...
    pub fn state(&self) -> ind_http_api::AppState {
        self.state.clone()
    }
    ...
    pub fn authed_client<'a, C: TestApiCredential>(
        &'a self,
        credential: &'a C,
    ) -> AuthedClient<'a> {
        AuthedClient {
    ...
    pub async fn create_extension_session(&self, user: &User) -> TestAuthSession {
        self.create_client_session(user, ClientType::Extension)
    }

    /// A JWT session for any client type, for tests that assert how an extractor
    /// treats web, mobile, desktop or CLI sessions differently.
    pub fn create_client_session(&self, user: &User, client_type: ClientType) -> TestAuthSession {
        TestAuthSession {
            user: user.clone(),
            token: self.sign_test_token(user, client_type),
        }
    }

    fn sign_test_token(&self, user: &User, client_type: ClientType) -> String {
        let (token, _expires) = ind_auth::sign_access_token(user.id, client_type, &self.jwt_secret)
