...
pub trait IntegrationOAuthProviderAdapter: Send + Sync {
    ...
    /// Revoke the grant upstream so the provider no longer honours the token.
    /// A token the provider already considers dead is a success: revocation is
    /// about reaching that state, and treating it as such keeps retries
    /// idempotent.
...
impl IntegrationOAuthService {
    ...
    pub async fn revoke(
        ...
        adapter.revoke_token(access_token).await
    }
