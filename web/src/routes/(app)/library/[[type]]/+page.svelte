<script lang="ts">
	...
	$effect(() => {
		if (!lib.filterBarOpen) return;
		const conds = lib.draftConditions;
		// Read the connector so switching And↔Or schedules a refetch; an effect only
		// re-runs for the values it actually read.
		void lib.draftConjunction;
